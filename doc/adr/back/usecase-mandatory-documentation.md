...
- Si le besoin de documentation des fonctionnalités est validée par les PO/UX alors il convient de documenter toutes les fonctionnalités en utilisant l'approche usecase + test unitaires même si en soit on ne test pas grand chose. C'est pour un besoin de documentation.
  - La mise en place du reporting automatique sera à faire mais le POC indique que la génération de rapport peut être:
    - rapide à configurer sur le repo
    - générer le rapport lors des pre-commit hook pour inclure le rapport autonome dans le repo, voir dans front/public pour y accéder en production (comme la doc api) 
- La documentation des parcours nécéssite plus de discutions avec PO/UX pour voir comment on peut faire. Pas sûr que les tests e2e soient pratiques mais ça ce discute et il faudra ajouter l'ensembles des parcours utilisateurs.
