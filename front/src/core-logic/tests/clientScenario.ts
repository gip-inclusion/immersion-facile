...
import { ClientTestApplication, ClientTestApplicationProperties } from "../../clientApplication/ClientApplication";
...
export function clientScenario(scenarioTitle: string, testSuite: ((application: ClientTestApplication) => void)[]) {
  ...
  describe(scenarioTitle, () => { testSuite.forEach(unitTest => unitTest(application)); });
}