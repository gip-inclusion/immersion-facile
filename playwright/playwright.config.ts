...
const baseURL = process.env.BASE_URL || `http://${domain}`;
...
export default defineConfig({
  ...
  webServer: {
    command: process.env.CI
      ? "pnpm --filter @pilotis/web build && pnpm --filter @pilotis/web start"
      : "pnpm --filter @pilotis/web dev --port 4000",
    url: baseURL,
    ...
    env: {
      ...
      NODE_ENV: "test",
      DATABASE_URL:
        "postgresql://immersion:password@localhost:5432/immersion-db",
