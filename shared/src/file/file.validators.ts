...
export const allowedFileSignatures: Record<
  AllowedExtension,
...
export const validateFile = (
  ...
  const matchingFileSignature = Object.values(allowedFileSignatures).find(
    ({ mimeType, extensions }) =>
      mimeType === file.type &&
  ...
  const matchingBytesSignature = Object.values(allowedFileSignatures).find(
    ({ bytes, extensions }) =>
      extensions.includes(extension as AllowedExtension) &&
      bytes.every((byte, index) => fileBytes[index] === byte),
  );

  if (!matchingBytesSignature) {
    return {
      code: "INVALID_SIGNATURE",
      message: "File content doesn't match its extension.",
    };
