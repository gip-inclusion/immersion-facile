...
describe("SendExchangeToRecipient", () => {
	...
	const discussionWithSiretAndAppelation = new DiscussionBuilder()
		...
		describe("sends the email to the right recipient", () => {
			it("when last exchange is from establishment, the exchange is sent to the potential beneficiary", async () => {
				...
				const discussion = new DiscussionBuilder(
					discussionWithSiretAndAppelation,
				)
		...
		describe("Specific scenarios", () => {
			it("skips the use case, if the skip parameter is provided", async () => {
				...
				await useCase.execute({
					...
					discussionId: discussionWithSiretAndAppelation.id,
				});
	...
	describe("wrong paths", () => {
		...
		it("throws an error if appellation code does not exist", async () => {
			const discussion = new DiscussionBuilder(discussionWithSiretAndAppelation)
				.withAppellationCode("20567")
		...
		it("throws an error if discussion does not have exchanges", async () => {
			uow.discussionRepository.discussions = [discussionWithSiretAndAppelation];

			await expectPromiseToFailWithError(
				useCase.execute({ discussionId: discussionWithSiretAndAppelation.id }),
				errors.discussion.noExchanges(discussionWithSiretAndAppelation.id),
			);
		});
