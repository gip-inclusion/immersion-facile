...
describe("PgAgencyRepository", () => {
  ...
  describe("closeInactiveAgenciesWithoutRecentConventions", () => {
    ...
    const oldConventionDate = subMonths(new Date(), 7).toISOString();
    const recentConventionDate = subDays(new Date(), 30).toISOString();
    ...
    it("closes only active agencies without recent conventions, excluding needsReview, rejected, and already closed", async () => {
      const activeAgency = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("aaaaaaaa-aaaa-4aaa-9aaa-aaaaaaaaaaaa")
          .withStatus("active")
          .build(),
        {
          [validator1.id]: { isNotifiedByEmail: true, roles: ["validator"] },
      ...
      const needsReviewAgency = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("bbbbbbbb-bbbb-4bbb-9bbb-bbbbbbbbbbbb")
          .withStatus("needsReview")
          .build(),
      ...
      const closedAgency = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("eeeeeeee-eeee-4eee-9eee-eeeeeeeeeeee")
          .withStatus("closed")
          .withStatusJustification("Already closed manually")
          .build(),
        {
          [validator1.id]: { isNotifiedByEmail: true, roles: ["validator"] },
        },
      ...
      const rejectedAgency = toAgencyWithRights(
        new AgencyDtoBuilder()
          ...
          .withStatus("rejected")
          .withStatusJustification("Rejected manually")
          .build(),
        {
          [validator1.id]: { isNotifiedByEmail: true, roles: ["validator"] },
        },
      ...
      await Promise.all([
        agencyRepository.insert(activeAgency),
        agencyRepository.insert(needsReviewAgency),
        agencyRepository.insert(closedAgency),
        agencyRepository.insert(rejectedAgency),
      ]);

      const closedAgencyIds =
        await agencyRepository.closeInactiveAgenciesWithoutRecentConventions({
          noConventionSince: sixMonthsAgo,
        });

      expectArraysToEqualIgnoringOrder(closedAgencyIds, [activeAgency.id]);

      expectToEqual(await agencyRepository.getById(activeAgency.id), {
        ...activeAgency,
        status: "closed",
        statusJustification: "Agence fermée automatiquement pour inactivité",
      });
      expectToEqual(
        await agencyRepository.getById(needsReviewAgency.id),
        needsReviewAgency,
      );
      expectToEqual(
        await agencyRepository.getById(closedAgency.id),
        closedAgency,
      );
      expectToEqual(
    ...
    it("closes agencies without recent conventions based on convention dates", async () => {
      const activeAgency1 = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("cccccccc-cccc-4ccc-9ccc-cccccccccccc")
          .withStatus("active")
          .build(),
      ...
      const activeAgency2 = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("dddddddd-dddd-4ddd-9ddd-dddddddddddd")
          .withStatus("active")
          .build(),
      ...
      const activeAgency3 = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("ffffffff-ffff-4fff-9fff-ffffffffffff")
          .withStatus("active")
          .build(),
        {
          [validator1.id]: { isNotifiedByEmail: true, roles: ["validator"] },
      ...
      const activeAgency4 = toAgencyWithRights(
        new AgencyDtoBuilder()
          .withId("aaaaaaaa-aaaa-4aaa-9aaa-aaaaaaaaaaaa")
          .withStatus("active")
          .build(),
      ...
      await Promise.all([
        agencyRepository.insert(activeAgency1),
        agencyRepository.insert(activeAgency2),
        agencyRepository.insert(activeAgency3),
        agencyRepository.insert(activeAgency4),
      ]);

      const oldConvention1 = new ConventionDtoBuilder()
        .withId("11111111-1111-4111-9111-111111111111")
        .withAgencyId(activeAgency2.id)
        .withDateSubmission(oldConventionDate)
        .build();

      const recentConvention1 = new ConventionDtoBuilder()
        .withId("22222222-2222-4222-9222-222222222222")
        .withAgencyId(activeAgency3.id)
        .withDateSubmission(recentConventionDate)
        .build();

      const oldConvention2 = new ConventionDtoBuilder()
        .withId("33333333-3333-4333-9333-333333333333")
        .withAgencyId(activeAgency4.id)
        .withDateSubmission(oldConventionDate)
        .build();

      const recentConvention2 = new ConventionDtoBuilder()
        .withId("44444444-4444-4444-9444-444444444444")
        .withAgencyId(activeAgency4.id)
        .withDateSubmission(recentConventionDate)
        .build();
      ...
      await Promise.all([
        conventionRepository.save(oldConvention1),
        conventionRepository.save(recentConvention1),
        conventionRepository.save(oldConvention2),
        conventionRepository.save(recentConvention2),
      ]);
      ...
      expectArraysToEqualIgnoringOrder(closedAgencyIds, [
        activeAgency1.id,
        activeAgency2.id,
      ]);

      expectArraysToEqualIgnoringOrder(
        await agencyRepository.getByIds([
          activeAgency1.id,
          activeAgency2.id,
          activeAgency3.id,
          activeAgency4.id,
