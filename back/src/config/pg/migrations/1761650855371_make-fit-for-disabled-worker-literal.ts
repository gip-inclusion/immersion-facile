...
export async function up(pgm: MigrationBuilder): Promise<void> {
  dropViews(pgm);
  pgm.createType("fit_for_disabled_workers_enum", [
    "yes-declared-only",
    "yes-ft-certified",
    "no",
  ]);
  ...
  pgm.renameColumn(
    ...
    "old__fit_for_disabled_workers",
  );

  pgm.addColumn("establishments", {
    fit_for_disabled_workers: {
      type: "fit_for_disabled_workers_enum",
    },
  });

  pgm.sql(`
    UPDATE establishments
    SET fit_for_disabled_workers =
      CASE
        WHEN old__fit_for_disabled_workers = true THEN 'yes-declared-only'::fit_for_disabled_workers_enum
        ELSE 'no'::fit_for_disabled_workers_enum
      END
  `);

  pgm.alterColumn("establishments", "fit_for_disabled_workers", {
    notNull: true,
  });

  pgm.dropColumn("establishments", "old__fit_for_disabled_workers");
  makeViews(pgm, "up");
}
...
export async function down(pgm: MigrationBuilder): Promise<void> {
  ...
  pgm.sql(`
    ...
    SET fit_for_disabled_workers =
      CASE
        WHEN old__fit_for_disabled_workers = 'yes-declared-only' THEN true
        WHEN old__fit_for_disabled_workers = 'yes-ft-certified' THEN true
        WHEN old__fit_for_disabled_workers = 'no' THEN false
        ELSE false
      END
  `);
  ...
  pgm.dropType("fit_for_disabled_workers_enum");
  makeViews(pgm, "down");
}
...
WITH count_conventions_by_siret AS (
 ...
 SELECT DISTINCT e.created_at AS "Date de référencement",
    ...
    e.number_employees AS "Nombre d’employés",
        ...
        CASE
            ${
              mode === "up"
                ? `
                WHEN e.fit_for_disabled_workers = 'yes-declared-only' THEN 'Oui - déclaré uniquement'::text
                WHEN e.fit_for_disabled_workers = 'yes-ft-certified' THEN 'Oui - certifié par FT'::text
              `
   ...
   FROM ((((((establishments e
     ...
     LEFT JOIN public_naf_classes_2008_old pnc ON (((pnc.class_id)::text = regexp_replace((e.naf_code)::text, '(\\d{2})(\\d{2}).*'::text, '\\1.\\2'::text))))
...
WITH offers_by_siret AS (
  ...
  LEFT JOIN public_appellations_data pad ON pad.ogr_appellation = io_1.appellation_code
)
...
WITH offers_by_siret AS (
  SELECT
    ...
    array_agg(pad.code_rome) AS rome_codes
  FROM establishments e
  LEFT JOIN immersion_offers io_1 ON io_1.siret = e.siret
  LEFT JOIN public_appellations_data pad ON pad.ogr_appellation = io_1.appellation_code
  GROUP BY e.siret
