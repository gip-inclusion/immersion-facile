...
export async function up(pgm: MigrationBuilder): Promise<void> {
  await recreateEstablishmentViews(pgm, "correct");
}

export async function down(pgm: MigrationBuilder): Promise<void> {
  await recreateEstablishmentViews(pgm, "withExistingBug");
}

const recreateEstablishmentViews = async (
  pgm: MigrationBuilder,
  conventionStatus: "correct" | "withExistingBug",
): Promise<void> => {
  ...
  WITH 
    count_conventions_by_siret AS (
        SELECT siret, count(*) 
        FROM conventions WHERE status ${
          conventionStatus === "correct"
            ? "= 'ACCEPTED_BY_VALIDATOR'"
            : "in ('VALIDATED', 'ACCEPETED_BY_VALIDATOR')"
        } 
        GROUP BY siret
        ),
