...
export class PgAgencyRepository implements AgencyRepository {
  ...
  public async getAll(): Promise<AgencyConfig[]> {
    const pgResult = await this.client.query(
      "SELECT id, name, address, counsellor_emails, validator_emails, admin_emails, questionnaire_url, email_signature, ST_AsGeoJSON(position) AS position\
  ...
  public async insert(config: AgencyConfig): Promise<AgencyId | undefined> {
    const query = `INSERT INTO public.agencies(
      id, name, address, counsellor_emails, validator_emails, admin_emails, questionnaire_url, email_signature, position
    ) VALUES (%L, %L, %L, %L, %L, %L, %L, %L, %s)`;
    try {
