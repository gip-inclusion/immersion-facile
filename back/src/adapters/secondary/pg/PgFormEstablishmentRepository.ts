  ...
  implements FormEstablishmentRepository
{
  ...
  public async update(
    ...
    //prettier-ignore
    const values = [
      siret, source, businessName, businessNameCustomized, businessAddress,
      isEngagedEnterprise, naf, JSON.stringify(appellations), JSON.stringify(businessContact), fitForDisabledWorkers,
      maxContactsPerWeek,
    ];
    const pgResult = await executeKyselyRawSqlQuery(
      this.transaction,
